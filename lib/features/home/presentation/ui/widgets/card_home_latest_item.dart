...
import '../../../../favorite/controller/favorite_event.dart';
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeBloc, HomeState>(
      buildWhen: (previous, current) =>
          previous.enHomeBestSeller != current.enHomeBestSeller,//less build when condition to only rebuild when the state changes
      builder: (context, state) {
        print('blo card best seller');
        if (state.enHomeBestSeller == EnState.loading) {
          return const Center(
            child: CircularProgressIndicator(color: ColorsManager.maincolor),
          );
        } else if (state.enHomeBestSeller == EnState.error) {
          return Center(
            child: Text(state.errorMessageBestSeller),
          );
        } else if (state.booksBestSeller.isEmpty) {
          return const Center(
        ...
        return SizedBox(
          ...
          child: ListView.builder(
            ...
            itemCount: state.booksBestSeller.length,
            itemBuilder: (context, index) {
              final book = state.booksBestSeller[index];
                    ...
                    InkWell(
                      onTap: () {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) =>  BookDetailsScreen(bookId: state.booksBestSeller[index].id),
                            ));
                      },
                      child: ClipRRect(
